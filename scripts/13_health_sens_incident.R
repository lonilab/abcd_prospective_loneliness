...
for (ith in 1:(length(health_variables))) {
    ...
    f1 <- glue::glue(
        "loneliness_fu_any ~ {health_var} + demo_sex_v2 + \
...
write_rds(
    health_res,
    here("outputs", "caches", "health_incident_res.rds")
)
...
ggsave(
    ...
    filename = here("outputs", "figures", "corr_health_incident_fig.pdf"),
    width = 8,
    height = 6,
    device = cairo_pdf
)
...
health_res |> 
    ...
    write_csv(here("outputs", "tables", "lmm_results_fu_incident_health.csv"))
