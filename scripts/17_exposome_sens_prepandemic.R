...
write_rds(
    exposome_res,
    here("outputs", "caches", "exposome_prepandemic_res.rds")
)
...
ggsave(
    plot = corr_env_fig,
    filename = here("outputs", "figures", "corr_env_prepandemic_fig.pdf"),
    width = 8,
    height = 6,
    device = cairo_pdf
)
...
exposome_res %>% 
    ...
    write_csv(
        here("outputs", "tables", "lmm_results_fu_prepandemic_environment.csv")
    )
