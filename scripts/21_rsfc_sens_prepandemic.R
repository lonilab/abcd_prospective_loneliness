...
master_rsfc_df <- selected_files %>%
    ...
    drop_na() |> # 7,344
    filter(first_loneliness_period %in% c("Never", "Prepandemic")) # 6,687
...
rsfc_lda_result$weights %>% 
    ...
    select(variable_name, from, to, mean_weight, ci_low, ci_high, is_sig) %>% 
     ...
     write_csv(here("outputs", "tables", "lda_results_prepandemic_rsfc.csv"))
...
ggsave(
    plot = corr_rsfc_fig,
    filename = here("outputs", "figures", "corr_rsfc_pandemic_fig.pdf"),
    width = 8,
    height = 6,
    device = cairo_pdf
)
