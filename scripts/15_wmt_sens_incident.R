...
for (ith in seq_along(wmt_labels)) { 
    ...
    formula_ <- glue::glue(
        "{region} ~ interview_age + demo_sex_v2 + \
...
wmt_lda_result$weights |> 
    ...
    write_csv(here("outputs", "tables", "lda_results_incident_wmm.csv"))

wmt_lda_orig_result <- read_csv(here("outputs", "tables", "lda_results_wmm.csv"))

corr_res <- cor.test(wmt_lda_orig_result$`LDA weight`, wmt_lda_result$weights$mean_weight)
...
ggsave(
    plot = corr_wmm_fig,
    filename = here("outputs", "figures", "corr_wmm_incident_fig.pdf"),
    width = 8,
    height = 6,
    device = cairo_pdf
)
