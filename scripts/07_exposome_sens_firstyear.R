...
for (ith in seq_along(exposome_variables)) {
    ...
    f1 <- glue::glue(
        "loneliness_firstyear_any ~ {env} + loneliness_bl + demo_sex_v2 + \
...
write_rds(
    exposome_res,
    here("outputs", "caches", "exposome_firstyear_res.rds")
)
...
ggsave(
    plot = corr_env_fig,
    filename = here("outputs", "figures", "corr_env_firstyear_fig.pdf"),
    width = 8,
    height = 6,
    device = cairo_pdf
)
...
exposome_res %>% 
    ...
    write_csv(
        here("outputs", "tables", "lmm_results_fu_firstyear_environment.csv")
    )
